...
사람인·원티드·자소설닷컴에서 채용 공고를 모아 조건에 맞는 것만 걸러내고, 관련성 순으로 정렬해주고,
...
```
① collect  — 사람인·원티드·자소설닷컴에서 오늘 공고를 가져와 조건에 맞는 것만 저장
...
②는 로컬 컴퓨터에서 도는 무료 AI 모델을 쓰고, ③·⑥은 Claude/API 중 설정한 AI를
...
- [ ] 사람인·원티드·자소설닷컴 회원가입은 **필요 없음** (공개 페이지/API만 사용, 로그인 없이 동작)
...
`collect`는 기본적으로 사람인·원티드에 더해 자소설닷컴에서도 공고를 가져옵니다 —
사람인·원티드가 거의 알려주지 않는 "실제 자소서 문항 유무"를 확인할 수 있는 유일한
소스입니다(§5 `collect` 설명 참고). 다만 이 소스는 **자소설닷컴 이용약관이 명시적으로
금지하는 자동화 수집 방식**을 씁니다(로그인 없이 공개 API만 호출하지만, 약관상 위반
소지가 있습니다). 이 리스크를 감수하고 싶지 않다면 `.env`에서 끄세요.

```bash
JASOSEOL_ENABLED=true   # 끄려면 false
```

기본값은 `true`입니다. `false`로 바꿔도 사람인·원티드 수집에는 아무 영향이 없습니다.
...
사람인·원티드·자소설닷컴에서 최신 공고를 가져와 `.env`의 `FILTER_*` 조건에 맞는 것만
...
같은 내용이 `output/run_log.txt`에도 누적 기록됩니다. 세 소스 중 하나가 요청 자체
실패로 0건이 됐거나(오늘 신규 공고가 정말 없어서 0건인 경우와는 구분됩니다), 사람인이
페이지 상한(10페이지)에 도달해 더 많은 공고가 있을 수 있으면 `[경고]`가 붙습니다.

> **자소설닷컴은 사람인/원티드가 거의 제공하지 않는 "실제 자소서 문항 유무"를 알려주는
> 소스입니다** — 회사가 자소설닷컴에 고정 질문형 자소서를 등록해뒀으면 `[자소서문항]`
> 줄로 표시됩니다(문항 원문은 API에 없어 링크를 열어 직접 확인해야 합니다, §7 `select`
> 참고). 처음 `collect`를 실행할 때는 이번 달에 열려있는 회사(400곳 이상)를 한 번에
> 조회하는 초기 비용이 있어 평소보다 오래 걸릴 수 있습니다 — 다음 실행부터는 새로 나타난
> 회사만 조회해 빨라집니다. 이용약관상 리스크와 끄는 방법은 [4-1a장](#4-1a-자소설닷컴-켜고-끄기-선택)
> 참고.
...
`RELEVANCE_ROLES`/`RELEVANCE_DOMAINS`를 설정했다면, 저장된 공고를 관련성 순으로 정렬해
...
`output/cover_letters/<ID>_<회사명>_<직무명>/materials/` 폴더가 자동으로 만들어지고,
그 안에 `notes.md` 빈 템플릿도 함께 자동 생성됩니다(이미 있으면 건드리지 않습니다). 이
폴더에 공고 스크린샷(이미지 파일)을 직접 넣어두거나 `notes.md`를 열어 내용을 채워두면,
다음 `write` 실행 시 자소서 계획 단계에서 참고합니다. 이미지 파일명은 아무렇게나 지어도
됩니다.

**실제 자소서 문항이 있다면 `notes.md`에 그대로 붙여넣어 두는 걸 강력히 권장합니다**
(문항 제목·글자수 제한까지 포함하면 더 좋습니다) — 없으면 AI가 표준 4문항(지원동기·직무
경험·강점·입사 후 포부)으로 가정해서 작성하지만, `notes.md`에 실제 문항이 있으면 그 문항을
최우선으로 따릅니다. `[출처] 자소설닷컴`이고 `[자소서문항]` 줄이 있는 공고라면 그 링크를
열어 실제 문항을 확인해 옮겨 적으세요(자소설닷컴 API는 문항이 있다는 사실만 알려줄 뿐
문구 자체는 제공하지 않습니다). `select` 실행 시 각 공고마다 이 경로와 안내를 콘솔에
출력합니다.

선택한 공고가 `[공채후보]`로 표시돼 있다면(아래 "파일과 마커 이해하기" 참고), `select`가
추가로 "세부 지원 직무를 `notes.md`에 적어두라"는 안내를 한 줄 더 출력합니다 — 이런
공고는 여러 직무를 한 번에 모집하는 공채일 수 있어, 실제로 어느 직무에 지원하는지 명시하지
않으면 AI가 뭉뚱그려 초안을 씁니다.
...
```
계획 수립 → 계획 평가 → (필요하면 계획 재작성)
  → 초안 작성 → 초안 평가 → (필요하면 초안 재작성 → 재평가, 최대 1회)
```
...
- saramin_54736780 작성 중...
  완료: output/cover_letters/saramin_54736780_카카오_Python 백엔드 개발자/draft.md
- wanted_380759 작성 중...
  완료: output/cover_letters/wanted_380759_주밍코리아_소프트웨어 R&D 기술기획/draft.md
자소서 작성 완료 — 성공 2건, 실패 0건. output/cover_letters/<ID>_<회사명>_<직무명>/ 하위에서
...
초안 평가에서 `NEEDS_REVISION`이 나오면 같은 `write` 실행 안에서 자동으로 한 번 더
다시 쓰고 재평가합니다. 그래도 `NEEDS_REVISION`이면 더 반복하지 않고(무한 재작성 방지)
남은 피드백과 함께 그 결과를 그대로 저장합니다 — `draft_review.md`가 여전히
`NEEDS_REVISION`이어도 정상 동작입니다.

**초안을 그대로 제출하지 말고, `draft_review.md`에 남은 지적 사항을 반영해 직접 다듬은 뒤
사용하세요.** 실사용해보면 자동 재작성 한 번을 거친 뒤에도 사소하지 않은 지적(예: 자격요건
미달을 자소서가 스스로 노출한다거나, 하필 필수 요건을 "약점"으로 골랐다거나)이 남는
경우가 흔하니, 남은 피드백을 가볍게 보지 마세요.
...
python jobfind.py select
# → 각 공고의 output/cover_letters/<ID>_<회사명>_<직무명>/materials/ 폴더 생성됨
#    (notes.md 빈 템플릿도 자동으로 함께 생성됨 — 원하면 여기에 스크린샷·메모 추가)

# 6. 자소서 초안 작성 (몇 분 소요)
python jobfind.py write

# 7. output/cover_letters/<ID>_<회사명>_<직무명>/draft.md 를 열어 검토·수정 후 사용
```
...
`[공채후보]` 줄은 두 번째 줄 마커와는 별개로, 제목에 "공채/부문별/직군별" 같은 신호가 있는
공고에 시스템이 자동으로 붙이는 정보성 필드입니다(사용자가 직접 편집하는 값이 아닙니다).
복수 직무를 한 번에 모집하는 공고일 수 있다는 뜻이며, 자동으로 직무를 나눠주지는 않습니다 —
`[자소서]`로 선택하면 `select`가 `notes.md`에 실제 지원 직무를 적어두라고 추가로 안내합니다.

`[자소서문항]` 줄도 마찬가지로 시스템이 자동으로 붙이는 정보성 필드입니다 — `[출처]
자소설닷컴`인 공고 중 자소서 고정질문이 등록된 직무가 있으면 그 직무명 목록과 함께
표시됩니다. 문항 원문은 이 줄에 없으니 링크를 열어 직접 확인해야 하며, `[자소서]`로
선택하면 `select`가 `notes.md`에 확인한 문항을 옮겨 적으라고 추가로 안내합니다.
...
| `output/cover_letters/<ID>_<회사명>_<직무명>/materials/` | 자소서 작성에 참고할 이미지·메모를 직접 넣는 곳(폴더명은 사람이 알아보기 쉽도록 회사명·직무명을 붙여 자동 생성됩니다) |
| `output/cover_letters/<ID>_<회사명>_<직무명>/*.md` | 자소서 작성 파이프라인의 결과물 |

---

## 8. 비용에 대해 알아둘 것

- **`collect`/`evaluate`/`add`/`select`는 완전 무료**입니다. 사람인·원티드·자소설닷컴은
  공개 페이지/API를 쓰고, 관련성 평가는 로컬에서 도는 오픈소스 모델을 씁니다.
- **`verify`(최종검수)와 `write`(자소서 작성)는 AI를 호출**하므로 쓰는 provider에 따라
  ...
  - `api:anthropic` / `api:openai`: 호출 1건마다 그대로 과금됩니다. 자소서 1건을 작성하는 데
    보통 4~7번의 AI 호출이 일어나므로(계획 → 계획평가 → [필요시 계획 재작성] → 작성 →
    초안평가 → [필요시 초안 재작성 → 재평가]), 공고 4개를 한 번에 처리하면 그만큼 호출이
    늘어난다는 점을 감안하세요.
...
| `collect` 실행 시 "조회: 0건" | 인터넷 연결 확인. 세 소스 중 일부만 실패해도 나머지는 정상 처리되니, 콘솔에 `[사람인]`/`[원티드]`/`[자소설닷컴]` 오류 메시지가 있는지 확인 |
| 첫 `collect` 실행이 평소보다 훨씬 오래 걸림 | 자소설닷컴 연동의 일회성 초기 비용입니다 — 이번 달에 열려있는 회사(보통 400곳 이상)를 한 번에 상세조회합니다. 다음 실행부터는 새로 나타난 회사만 조회해 빨라집니다 |
...
| 자소서 초안 맨 앞에 "공고 원문을 확인하지 못해 표준 구성으로 작성했습니다" 같은 안내가 있음 | 정상 동작입니다 — 없는 정보를 지어내지 않고 가정한 부분을 밝힌 것. `output/cover_letters/<ID>_<회사명>_<직무명>/materials/notes.md`에 실제 자소서 문항을 붙여넣고 `write`를 다시 실행하면 그 문항을 최우선으로 반영합니다. 그래도 본문은 끝까지 작성되니 그대로 참고하되, `draft_review.md`의 지적과 함께 실제 공고 내용으로 다듬으세요 |
...
| 출력 | UTF-8 txt (`jobs_all.txt`) + 공고별 자소서 결과 (`cover_letters/<ID>_<회사명>_<직무명>/`) |
...
- 자소설닷컴 연동은 "이 회사에 자소서 고정질문이 있다"는 사실과 어느 직무인지까지만
  자동으로 알려줍니다(`[자소서문항]` 줄) — 문항 원문·글자수 제한은 API에 없어 링크를 열어
  직접 확인해야 합니다. 확인한 내용은 `materials/notes.md`에 복사해 넣으세요 — 있으면
  계획·작성 단계가 표준 4문항 가정보다 이 문항을 최우선으로 따릅니다. 또한 이 연동은
  자소설닷컴 이용약관의 자동 수집 금지 조항과 상충될 수 있다는 점을 알고 사용하세요
  ([4-1a장](#4-1a-자소설닷컴-켜고-끄기-선택)에서 끌 수 있습니다).
